...
def get_referrals(user_id):
    ...
    :param user_id: id пригласившего
    :return:
    """
    with connect_db() as db:
        cur = db.cursor()
        return cur.execute("SELECT * FROM UserList WHERE inviting = ?", [user_id]).fetchall()
...
def add_buy(purchase_data):
    ...
    with connect_db() as db:
        cur = db.cursor()
        cur.execute("INSERT INTO Sales (user_id, item_name, amount, count, date, cheque, payment) "
                    "VALUES (?, ?, ?, ?, ?, ?, ?)",
...
def find_purchase(cheque):
    ...
    with connect_db() as db:
        cur = db.cursor()
        return cur.execute("SELECT * FROM Sales WHERE cheque = ?", [cheque]).fetchone()


def get_purchase_items(sale_id):
    """
    Все товары в покупке
    ...
    with connect_db() as db:
        cur = db.cursor()
        return cur.execute("SELECT * FROM SoldItems WHERE sale_id = ?", [sale_id]).fetchall()
...
def get_qiwi():
    ...
    with connect_db() as db:
        cur = db.cursor()
        return cur.execute("SELECT * FROM Qiwi").fetchall()


def add_qiwi(qiwi_data):
    """
    ...
    with connect_db() as db:
        ...
        cur.execute("INSERT INTO Qiwi (num, token, nickname) VALUES (?, ?, ?)",
                    (qiwi_data['num'], qiwi_data['token'], qiwi_data['nickname']))
