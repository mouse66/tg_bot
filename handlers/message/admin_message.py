...
from bin.purchase.find_purchase import purchase_cheque
from bin.users import user_finder
from bin.keyboards import create_list_keyboard, CLOSE_BTN
...
async def qiwi_edit(message: types.Message):
    ...
    """
    keyboard = types.InlineKeyboardMarkup()
    keyboard.add(types.InlineKeyboardButton(text=const_ru["add_payment"],
                                            callback_data="add_qiwi"))
    qiwi = database.get_qiwi()
    if qiwi is None:
        keyboard.add(keyboards.CLOSE_BTN)

        await message.answer("😔 Кошельки отсутствуют\n<b>Добавьте новый кошелек</b>",
                             reply_markup=keyboard)
    else:
        message_text = f"💳 Количество кошельков: <b>{len(qiwi)} шт.</b>"
...
@dp.message_handler(IDFilter(chat_id=ADMIN_ID), regexp=const_ru["banker"])
async def yoomoney_edit(message: types.Message):
    """
    Управление BTC Banker
    ...
    keyboard = types.InlineKeyboardMarkup()
    keyboard.add(types.InlineKeyboardButton(text=const_ru["edit_payment"],
                                            callback_data="edit_banker"))
    keyboard.add(types.InlineKeyboardButton(text=const_ru["check_payment"],
                                            callback_data="check_banker"))
    keyboard.add(keyboards.CLOSE_BTN)
...
async def statistics(message: types.Message):
    ...
    keyboard.row(const_ru['find_purchase'])
    keyboard.row(const_ru["back"])
...
@dp.message_handler(IDFilter(chat_id=ADMIN_ID), regexp=const_ru["find_purchase"])
async def find_purchase(message: types.Message):
    """
    Поиск покупки
...
async def users(message: types.Message):
    ...
    keyboard.row(const_ru['referral_program'])
    keyboard.row(const_ru['back'])
...
@dp.message_handler(IDFilter(chat_id=ADMIN_ID), regexp=const_ru['referral_program'])
async def referral_program(message: types.Message):
    ...
    :return:
    """
    message_text = f"Процент реферальной программы: <b>{database.get_param('referral_percent')}%</b>"
    keyboard = types.InlineKeyboardMarkup()
    keyboard.add(types.InlineKeyboardButton(text=const_ru['edit'], callback_data="edit_ref_percent"))
...
async def active_support(message: types.Message):
    ...
    keyboard = create_list_keyboard(data=database.get_supports(0),
                                    last_index=0,
                                    page_click=f"get_user_support=0",
...
async def closed_support(message: types.Message):
    ...
    keyboard = create_list_keyboard(data=database.get_supports(1),
                                    last_index=0,
                                    page_click=f"get_user_support=1",
                                    btn_text_param="support",
                                    btn_click="get_support")
