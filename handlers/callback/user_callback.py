...
from bin.balance.balance import amount_balance
...
async def get_user_supports(call: types.CallbackQuery):
    ...
    await call.message.answer(const_ru["my_support"], reply_markup=keyboard)
...
@dp.callback_query_handler(Regexp("add_balance"))
async def add_balance(call: types.CallbackQuery):
    ...
    """
    await call.message.delete()
    await amount_balance(call.message)


# # # Реферальная программа # # #

@dp.callback_query_handler(Regexp("referral_program"))
async def referral_program(call: types.CallbackQuery):
    """
    Реферальная программа
    ...
    message_text = f"📧 Реферальная программа\n\n" \
                   f"Ссылка для приглашения: https://t.me/{bot_data['username']}?start={call.message.chat.id}\n" \
                   f"Количество приглашенных пользователей: " \
                   f"{len(database.get_referrals(call.message.chat.id))}\n\n" \
                   f"С каждого приглашенного пользователя вы получаете " \
                   f"<b>{database.get_param('referral_percent')}%</b> с каждой его покупки"
