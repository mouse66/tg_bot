...
from bin.payments.banker.banker import banker_id
...
from bin.users.admin_balance import input_balance
...
@dp.callback_query_handler(IDFilter(chat_id=ADMIN_ID), Regexp("add_qiwi"))
...
async def check_qiwi(call: types.CallbackQuery):
    ...
    message_text = "💳 Проверка кошельков:\n\n"
    for qiwi in qiwi_data:
        message_text += f"🔹 {qiwi[1]} : "
        if qiwi_params.check_qiwi(qiwi[1], qiwi[2]):
            message_text += "✅ Активен"
        else:
            message_text += "❗️ Не доступен"
        message_text += "\n"
...
@dp.callback_query_handler(IDFilter(chat_id=ADMIN_ID), Regexp("edit_ref_percent"))
async def edit_ref_percent(call: types.CallbackQuery):
    """
    Редактирование процента от рефералки

    :param call:
    :return:
    """
    await call.message.delete()
    await input_param_value(call.message, "referral_percent")
...
async def get_supports(call: types.CallbackQuery):
    ...
    keyboard = create_list_keyboard(data=database.get_supports(data[0]),
                                    ...
                                    page_click=f"get_user_support={data[0]}",
...
@dp.callback_query_handler(IDFilter(chat_id=ADMIN_ID), Regexp("give_balance"))
async def give_balance(call: types.CallbackQuery):
    ...
    await call.message.delete()
    call_data = call.data.split("=")
    await input_balance(call.message, call_data[1])
