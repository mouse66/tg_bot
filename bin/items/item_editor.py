...
async def get_item_data_info(message, item_id):
    ...
    message_text = "📋 Тип товара: <b>{type_data}</b>\n"

    if item_type[0] == "text":
        type_data = "Текст"
        await message.answer(message_text.format(type_data=type_data) + f"📦 Данные:\n\n<b>{item_type}</b>",
                             reply_markup=keyboard)
    else:
        type_data = "Файл"
        await message.answer_document(item_type[1],
                                      message_text.format(type_data=type_data),
                                      reply_markup=keyboard)
