...
def create_qiwi_link(amount, comment):
    ...
    :return:
    """
    qiwi_data = get_qiwi_work()
    if qiwi_data is None:
        return None
...
def check_qiwi_payment(amount, comment):
    ...
    request.headers["authorization"] = f"Bearer {qiwi_data[0][2]}"
    params_get = {"rows": 5, "operation": "IN"}
    response = request.get(
        f"https://edge.qiwi.com/payment-history/v2/persons/{qiwi_data[0][1]}/payments",
        params=params_get)
...
def get_qiwi_work():
    """
    Получение рабочего QIWI

    :return:
    """
    count = 0
    while True:
        qiwi = random.choice(database.get_qiwi())
        if check_qiwi(qiwi[1], qiwi[2]):
            return qiwi
        count += 1

        if count == len(database.get_qiwi()):
            return None
